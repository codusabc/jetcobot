...
def generate_launch_description():
    moveit_config = (
        MoveItConfigsBuilder(
            robot_name="jetcobot", package_name="jetcobot_moveit_config"
        )
        .robot_description(file_path="config/firefighter.urdf.xacro")
        .trajectory_execution(file_path="config/moveit_controllers.yaml")
        .moveit_cpp(
            file_path=get_package_share_directory("jetcobot_moveit_picker")
    ...
    static_tf = Node(
        ...
        output="log",
        arguments=["--frame-id", "world", "--child-frame-id", "g_base"],
    )
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory("bronze_moveit_config"),
        "config",
