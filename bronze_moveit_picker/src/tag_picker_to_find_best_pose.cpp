...
public:
    ...
    bool execute()
    {
        ...
        if (stored_tag_transforms_.empty()) {
            RCLCPP_ERROR(get_logger(), "No stored tag transforms available for pick and place operations.");
            return false;
        }

        RCLCPP_INFO(get_logger(), "Successfully prepared for pick operation!");

        // Pick operation
        int pick_tag_id = stored_tag_transforms_.begin()->first;  // Use the first stored tag for picking
        RCLCPP_INFO(get_logger(), "Starting pick operation for tag ID: %d", pick_tag_id);
        if (!executePick(pick_tag_id)) {
            RCLCPP_ERROR(get_logger(), "Pick operation failed for tag ID: %d", pick_tag_id);
            return false;
        }
        
        RCLCPP_INFO(get_logger(), "Complete !");
